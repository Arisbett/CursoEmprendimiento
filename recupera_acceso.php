...
$grupos = select("id_estado, estado", "states", "estado!='' ", "id_estado");
//var_export($grupos);
?>
...
<title>CONDUSEF | CONDUSEF | CURSO DE EMPRENDIMIENTO - RECUPERA ACCESO</title>
...
<ol class="breadcrumb">
			...
			<li><a href="index.php">Recupera acceso - CONDUSEF | CURSO DE EMPRENDIMIENTO</a></li>
		</ol>
...
<div class="form-group">
    <label class="col-sm-5 control-label" for="grupo-acceso">Entidad Federativa<span id="asterisco_grupo">*</span>:</label>
    <div class="col-sm-7">
        <select class="form-control" name="grupo" id="grupo">
            <option value="0">
            <?php for($i=0;$i<count($grupos);$i++){ ?>
                <option value="<?php echo $grupos[$i]["id_estado"]; ?>"><?php echo $grupos[$i]["estado"]; ?>
            <?php } ?>
        </select>
        
    <div id="err_asterisco_grupo"></div>
    </div>
</div>
...
function checar_forma() {
    ...
    if(form.grupo.value=="0"){
          campo_error("grupo",1,"");
          texto_error("asterisco_grupo",1,"");
          error_msg = 1;
    } else {
        campo_error("grupo",2,"msg");
        texto_error("asterisco_grupo",2,"msg");
    }
    ...
    form.method="post";
    form.action="recupera_acceso_envia.php";
    form.submit();
}

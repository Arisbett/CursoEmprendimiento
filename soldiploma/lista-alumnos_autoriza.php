...
if($cextranio==0){
    ...
    for($i=0;$i<count($lista);$i++){
        if(is_numeric($lista[$i])){
            ...
            $autorizacion = quitar_autorizacion_alumno($id_generacion,$id_alumno,$id_grupo,$id_coordinador);
            } else{ //print "<br>autorizo $id_generacion,$id_alumno,$id_grupo,$id_coordinador";
            $autorizacion = autoriza_alumno($id_generacion,$id_alumno,$id_grupo,$id_coordinador);
            }
            //echo '<br>id:'.$lista[$i];
            $total += $autorizacion;
        }

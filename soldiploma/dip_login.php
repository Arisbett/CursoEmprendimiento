...
if($usuario && $pswd){
    ...
    $info_usuario = select("id_coordinador,idGrupo,nombre,password,id_tipo_coordina", "coordinadores", "activo='1' AND usuario='$usuario'", "", ""); 
    $pass_b = $info_usuario[0]["password"];
...
//var_export( $id_coordinador);

     if(verifica_pass($pswd,$pass_b)==1){
                $id_usuario = $info_usuario[0]["id_coordinador"];
                //var_export($id_usuario);
     }else{
                $id_usuario = 0;}
    ...
    if($id_usuario>0){
        ...
        $_SESSION["fecha1_registro"]    = "20-03-2023"; //"22-08-2022";
        $_SESSION["fecha2_registro"]    = "2023-03-30";
        $_SESSION["id_generacion"]  =   "1";
        $_SESSION["id_super_admin"]  =   1;
        $_SESSION["id_tabla_generacion"] = 1;

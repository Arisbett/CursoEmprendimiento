...
$reg=1;
    ...
    if($_POST["correo"] != $_POST["correo2"]) {
      ...
      $verifica = Conexion::conectar()->prepare("SELECT * FROM alumno WHERE correo = :correo and registros=2");
      $verifica->bindParam(':correo', $correo, PDO::PARAM_STR);
      $verifica->execute();
      $total = $verifica->rowCount();
      
      if ($total == 1) {
        $query = Conexion::conectar()->prepare("SELECT * FROM grupo WHERE idGrupo = :idGrupo");
        ...
        $query = Conexion::conectar()->prepare("INSERT INTO alumno(nombre, apaterno, amaterno, correo, pass, sexo, estado, edad, idGrupo, idGeneracion, informacion, registros)
                                                            VALUES (:nom, :apat, :amat, :correo, :pass, :sex, :estado, :edad, :idGrupo, :gen, :info, :reg+1)" );

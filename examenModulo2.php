...
if($_POST){
    ...
    for($i=1;$i<=10;$i++){
        ...
        $nuevowhere .= "SELECT * FROM actividad_preguntas WHERE id_pregunta=".${"id_pregunta".$i}.($i<10?" union ":")");
        ...
        <p><strong>¡Una vez habilitado el examen se deberá de responder de manera continua, de lo contrario la sesión expirará y se tendrá que realizar nuevamente!</strong></p></div>
            <div><p>La presente evaluaci&oacute;n permite evidenciar los conocimientos adquiridos en el M&oacute;dulo II, cuenta con el 60% que se suma a la calificación final.</p>
            ...
            <form role="form" name="examen">
                ...
                <?php
                    ...
                    foreach($resultado AS $row){ 
                        ...
                        echo '<div class="form-group"> <label>'.$row ["pregunta"].'<span id="asterisco_r'.$i.'" class="form-text">*</span>:</label>';
                        
                        echo '<div class="checkbox">';
                        $comando1 = Conexion::conectar()->query("SELECT id_opciones,id_pregunta, opcion FROM actividad_opciones WHERE id_actividad='2' AND id_pregunta=$row[id_pregunta]");
                        $comando1->execute();
...
//alert(document.getElementById("r1_1")+"*"+document.getElementById("r1_2")+"*"+document.getElementById("r1_3"))
   ...
   form.submit();
    ...
    function myFunction() {
  confirm("¿Estas seguro de enviar el examen del Módulo II?");
}
    </script>
...
<?php
    ...
    if($_POST){    
        ...
        for($i=1;$i<=10;$i++){
            ...
            $camposr .= "Respuesta".$i.($i<10?",":"");
            $valoresr .= ${"id_respuesta".$i}.($i<10?",":"");
    
            $camposp .= "Pregunta".$i.($i<10?",":"");
            $valoresp .= ${"id_pregunta".$i}.($i<10?",":"");
             //echo '<br>'.$i.' id_pregunta:<b>'. ${"id_pregunta".$i}.'</b> respuesta:<b>'.${"id_respuesta".$i}.'</b>'.($correctas[${"id_pregunta".$i}]==${"id_respuesta".$i}?"Bien":"Mal");
    ...
    $calEx=60*$total_bien/10;
    if ($total_bien==10) {
        echo "<div>&nbsp;</div><div>&nbsp;</div><div class=\"alert alert-success\"><b>Tienes " .$total_bien." respuestas correctas de 10 preguntas.<br> Obtuviste el $calEx % de la calificación final</b></div>";
    
     } else{
        echo "<div>&nbsp;</div><div>&nbsp;</div><div class=\"alert alert-warning\"><b>Tienes " .$total_bien." respuestas correctas de 10 preguntas.<br> Obtuviste el $calEx % de la calificación final</b></div>";
                           
    }
     //echo "INSERT INTO examen_m1 (idAlumno,$camposr,$camposp,Calificacion,idGeneracion) VALUES (:idAl, $valoresr,$valoresp, :cal, :idGen)";
    $query = Conexion::conectar()->prepare("INSERT INTO examen_m2 (idAlumno,$camposr,$camposp,Calificacion,idGeneracion) VALUES (:idAl, $valoresr,$valoresp, :cal, :idGen)");

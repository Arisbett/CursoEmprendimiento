    ...
    for($i=1;$i<=15;$i++){
        ...
        $nuevowhere .= "SELECT * FROM actividad_preguntas WHERE id_pregunta=".${"id_pregunta".$i}.($i<15?" union ":")");
         //echo '<br>'.$i.' id_pregunta:<b>'. ${"id_pregunta".$i}.'</b> respuesta:<b>'.${"id_respuesta".$i}.'</b>'.($correctas[${"id_pregunta".$i}]==${"id_respuesta".$i}?"Bien":"Mal");
            ...
            $comando = Conexion::conectar()->query("SELECT * FROM actividades WHERE id_actividad=16");
            $comando->execute();
        ...
        <p><strong>¡Una vez habilitado el examen se deberá de responder de manera continua, de lo contrario la sesión expirará y se tendrá que realizar nuevamente!</strong></p></div>
            <div><p>La presente evaluaci&oacute;n permite evidenciar los conocimientos adquiridos en el M&oacute;dulo III, cuenta con el 60% que se suma a la calificación final.</p>
            ...
            <form role="form" name="examen">
                ...
                <?php
                    if(!$nuevowhere){ 
                    $comando = Conexion::conectar()->query("SELECT * FROM actividad_preguntas WHERE id_actividad='16' ORDER BY rand() LIMIT 15");
                    } else { 
                    ...
                    foreach($resultado AS $row){ 
                        ...
                        $comando1 = Conexion::conectar()->query("SELECT id_opciones,id_pregunta, opcion FROM actividad_opciones WHERE id_actividad='16' AND id_pregunta=$row[id_pregunta]");
                        $comando1->execute();
...
//alert(document.getElementById("r1_1")+"*"+document.getElementById("r1_2")+"*"+document.getElementById("r1_3"))
   ...
   form.submit();
    ...
    function myFunction() {
  confirm("¿Estas seguro de enviar el examen del Módulo III?");
}
    </script>
...
<?php
    ...
    $query = Conexion::conectar()->prepare("INSERT INTO examen_m3 (idAlumno,$camposr,$camposp,Calificacion,idGeneracion) VALUES (:idAl, $valoresr,$valoresp, :cal, :idGen)");

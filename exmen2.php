...
if(empty($r1)){
   ...
   $query = Conexion::conectar()->prepare("SELECT idAlumno FROM examendiag WHERE idAlumno =:idAl");
              ...
              if ($total>=1) {
               //echo '<script>alert("Ya respondiste el Examen Diagnostico")</script>';
               $mensaje = "Ya respondiste el Examen Diagnostico";
        ...
        $query->bindParam(':idGen', $idGen, PDO::PARAM_STR);
             ...
             $query->execute();
              ...
              <p class="pull-right"><button class="btn btn-primary" type="button" onClick="document.location.href=\'Modulos.php?p=1\'">Comenzar M&oacute;dulo I</button>
              </p>
              </div>
          </div>';   

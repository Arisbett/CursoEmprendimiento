...
<p>Obtenido este resultado, podrás pasar a cursar el siguiente nivel. </p>
            ...
            <form role="form" name="actividad7">
                <div class="form-group pregunta1">
                             <?php
                             $comando = $con->query("SELECT pregunta FROM actividad_preguntas WHERE id_pregunta=1");
                             $comando->execute();
                             $resultado = $comando->fetchAll(PDO::FETCH_ASSOC);
                                ?>
                         <?php foreach($resultado AS $row){?>
...
<div class="form-group">
	...
	<p class="pull-right">    
        ...
        <button class="btn btn-primary" type="button"  onClick="return fenvia()" id="valida">Enviar</button>
       
        <!--<button class="btn btn-primary" type="button" onClick="fenvia()">Validar</button>-->
    </p>
    </div>
</div>
...
if(tipo==1){
    ...
    document.getElementById("err_"+element).innerHTML='';
           ...
           $suma = $puntaje_1 + $puntaje_2 + $puntaje_3 + $puntaje_4 + $puntaje_5 + $puntaje_6 + $puntaje_7 + $puntaje_8 + $puntaje_9;
           $por=round(10*$suma/9);
                      ...
                      $idAl=$_SESSION['idAlumno'];
                            ...
                            $calificacion_actividad1 = $resultadoA[0]["intentos"];

           if ($calificacion_actividad1>=4) {
             echo "<div class=\"alert alert-info\"><strong>Obtuviste ".$suma." respuestas correctas de 9 preguntas.
             </strong> </div>";
             echo "<script>document.getElementById('valida').style.visibility = 'hidden';</script>";
             //echo "<script>document.getElementById('valida').style.visibility = 'hidden';</script>";
 
          } else{
             echo "<div class=\"alert alert-warning\"> <strong>Obtuviste ".$suma." respuesta correcta de 9 preguntas. <br>
             </strong> </div>";
             //Para que la actividad sea promediada, se requiere que TODAS LAS RESPUESTAS SEAN CORRECTAS, si es necesario vuelva a leer la información e intente nuevamente las veces que sea necesario.
                ...
                $query = Conexion::conectar()->prepare("SELECT idAlumno FROM actividad1_m1 WHERE idAlumno =:idAl");
              $query->bindParam(':idAl', $idAl, PDO::PARAM_STR);
              $query->execute();
              $total = $query->rowCount();

              if ($total>=1) {
                $query = Conexion::conectar()->prepare("UPDATE actividad1_m1 SET Respuesta1 = :r1, Respuesta2 = :r2, Respuesta3 = :r3, Respuesta4 = :r4, Respuesta5 = :r5, Respuesta6 = :r6, Respuesta7 = :r7, Respuesta8 = :r8, Respuesta9 = :r9, 
                Calificacion = :cal, fecha_registro=NOW(), intentos = (intentos + 1) WHERE actividad1_m1.IdAlumno = :idAl");
                ...
                $query->bindParam(':idAl', $idAl, PDO::PARAM_STR);
                //$query->bindParam(':intent', $contador, PDO::PARAM_STR);
                $query->execute();


              }else {
                $query = Conexion::conectar()->prepare("INSERT INTO actividad1_m1 (Respuesta1, Respuesta2, Respuesta3, Respuesta4, Respuesta5, Respuesta6, Respuesta7, Respuesta8, Respuesta9, Calificacion,idAlumno, idGeneracion, intentos) VALUES(:r1,:r2,:r3,:r4,:r5,:r6,:r7,:r8,:r9,:cal,:idAl,:idGen, :intentos)");
                $query->bindParam(':r1', $r1, PDO::PARAM_STR);
                $query->bindParam(':r2', $r2, PDO::PARAM_STR);
                $query->bindParam(':r3', $r3, PDO::PARAM_STR);
                $query->bindParam(':r4', $r4, PDO::PARAM_STR);
                ...
                $id_generacion=1;
                if ($totalc>=1) {
                    $queryAc = Conexion::conectar()->prepare("UPDATE `boleta` SET `Calificacion1` = :cal WHERE boleta.idAlumno = :idAl");
                $queryAc->bindParam(':cal', $por, PDO::PARAM_STR);
                $queryAc->bindParam(':idAl', $idAl, PDO::PARAM_STR);
                $queryAc->execute();
                }else{
                    $queryIn = Conexion::conectar()->prepare("INSERT INTO boleta (idAlumno, idGeneracion, Calificacion1) VALUES(:idAl,1,:cal)");
                    $queryIn->bindParam(':idAl', $idAl, PDO::PARAM_STR);
                    $queryIn->bindParam(':id_Gen', $id_generacion, PDO::PARAM_STR);
